...
func trigger_chromatic_aberration(duration: float = 0.5, intensity: float = 0.005) -> void:
	...
	_tween.tween_method(
		func(val): _material.set_shader_parameter("aberration", val), 0.0, intensity, duration * 0.2
	)
	_tween.tween_method(
		func(val): _material.set_shader_parameter("aberration", val), intensity, 0.0, duration * 0.8
	)
