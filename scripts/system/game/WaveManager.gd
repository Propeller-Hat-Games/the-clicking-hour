...
func next_wave() -> void:
	...
	if game.door: game.door.open(SfxManager)
	print("[WAVE] Wave %d started!" % game.current_wave)
	if game.is_night_mode: print("[WAVE] This wave is night mode!")
...
func end_game() -> void:
	...
	game_over_instance.set_waves_survived(game.current_wave - 1, game.entities_killed, game.glass_passed)
