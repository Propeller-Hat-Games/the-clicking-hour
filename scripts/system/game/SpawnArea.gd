...
func is_position_valid(pos: Vector2, min_distance_squared: float, ignore_entity: Entity = null) -> bool:
...
func get_valid_random_position(ignore_entity: Entity = null, min_dist: float = 250.0, max_attempts: int = 50) -> Vector2:
...
func spawn_entity(game: GameManager) -> void:
	...
	entity.set_speed(entity.get_speed() * (game.current_wave / 3 + 1))
