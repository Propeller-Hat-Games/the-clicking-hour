...
func set_pause_effect(paused: bool) -> void:
	...
	(
		_pause_tween
		. parallel()
		. tween_property(_low_pass_effect, "cutoff_hz", target_cutoff, 0.5)
	...
	(
		_pause_tween
		. parallel()
		. tween_property(self, "volume_db", target_volume, 0.5)
		. set_trans(Tween.TRANS_SINE)
		. set_ease(Tween.EASE_OUT)
	)

	if not paused:
		_pause_tween.finished.connect(
			func():
				AudioServer.set_bus_effect_enabled(_music_bus_index, _low_pass_effect_index, false)
		)
...
func fade_out() -> void:
	...
	(
		_fade_tween
		. tween_property(self, "volume_db", -80.0, _fade_duration)
		. set_trans(Tween.TRANS_SINE)
...
func fade_to(next_stream: AudioStream) -> void:
	...
	if playing:
		(
			_fade_tween
			. tween_property(self, "volume_db", -80.0, _fade_duration / 2.0)
			. set_trans(Tween.TRANS_SINE)
			. set_ease(Tween.EASE_IN)
		)
	else:
	...
	if next_stream != null:
		(
			_fade_tween
			. tween_property(self, "volume_db", 0.0, _fade_duration / 2.0)
			. set_trans(Tween.TRANS_SINE)
			. set_ease(Tween.EASE_OUT)
		)
